...
Эквивалентная часто используемая форма: непрерывное отображение компактного выпуклого множества в себя имеет неподвижную точку.
...
![Относительно компактный образ покрывается конечной сетью, приближённые неподвижные точки сходятся к точному пределу; без компактности последовательность может уйти](80_assets/theorems/gpt-image-v3/schauder-compactness-fixed-point-v3.png)

> Конечная сеть переводит задачу в конечномерную геометрию, а компактность возвращает предельную точку в исходное пространство. Теорема доказывает существование, но не задаёт сходящийся алгоритм.
