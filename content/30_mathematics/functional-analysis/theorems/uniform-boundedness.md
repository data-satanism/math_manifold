...
> [!info] analogy
> Для семейства линейных производных поточечная ограниченность по каждому направлению может превратиться в общую оценку операторных норм только при выполнении условий теоремы. Проверка конечного набора входов такой гарантии не даёт; для нелинейных моделей область линейзации и полноту пространства надо задавать явно.

## Визуальная схема

![Локальная общая граница для семейства линейных операторов масштабируется на весь единичный шар; конечная проверка якобианов не заменяет равномерную оценку](80_assets/theorems/gpt-image-v3/uniform-boundedness-local-to-global-v3.png)

> Теорема превращает локальный шар, найденный с помощью Бэра, в единую границу норм по всем направлениям. Нижняя полоса показывает предел переноса на нелинейные отображения.
